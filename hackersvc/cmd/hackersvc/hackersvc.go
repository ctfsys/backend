...
import (
	...
	hackerpb "github.com/ctfsys/backend/hackersvc/pb"
...
func main() {
	// We'll chose one transport and one tracer, but we try out everything
	// now, just because we can.

	fs := flag.NewFlagSet("hackersvc", flag.ExitOnError)
	var (
	...
	var tracer stdopentracing.Tracer
	{
		if *zipkinURL != "" {
			...
			logger.Log("tracer", "LightStep") // probably don't want to print the token :)
	...
	var pings metrics.Counter
	{
		// Business-level metrics
		pings = prometheus.NewCounterFrom(stdprometheus.CounterOpts{
			Namespace: "ctfsys",
			Subsystem: "hackersvc",
			Name:      "pings_requested",
			Help:      "Total number of pings asked for via the Ping method.",
		}, []string{})
	}
	...
	var (
		service      = hackerservice.New(logger, pings)
		endpoints    = hackerendpoint.New(service, logger, duration, tracer)
		httpHandler  = hackertransport.NewHTTPHandler(endpoints, tracer, logger)
		grpcServer   = hackertransport.NewGRPCServer(endpoints, tracer, logger)
		thriftServer = hackertransport.NewThriftServer(endpoints)
	)
	...
	// interrupts the furst function and cause it to return. It's in these
	...
	{
		// The HTTP listener mounts the HTTP handler.
	...
	{
		// The gRPC listener mounts the gRPC server.
		...
		g.Add(func() error {
			...
			hackerpb.RegisterHackerServer(baseServer, grpcServer)
	...
	{
		// The Thrift socket mounts the Go kit Thrift server.
		...
		g.Add(func() error {
			...
			return thrift.NewTSimpleServer4(
				hackerthrift.NewHackerServiceProcessor(thriftServer),
				thriftSocket,
				transportFactory,
				protocolFactory,
			).Serve()
